<?php
/** Name: M2mResponse
*  Description: Object gets response from EE M2M Connect service using SOAP w/ WSDL integration. Message is in the format: gobbwobblers(0,1,0,1,0,32,2468) as described in documentation
...
class M2mResponse
{
    ...
    function __construct()
	{
		...
		try
		{
			...
			//loop through results to get newest gobbwobblers message
			if (isset($soap_results))
			{
				for ($i = 0;$i < count($soap_results);$i++)
				{
					//set the var lastMsg, if a message containing "gobbwobblers" is found
					if (strpos($soap_results[$i], "gobbwobblers") > 0)
					{
			...
			else
			{
				...
				$this->error .= "No message found for gobbwobblers! Make sure the correct M2MConnect details are in src/M2mResponse.php variables, and send a message to your MSISDN in this format: gobbwobblers(0,0,0,0,0,99,9999)";
				return false;
			}
